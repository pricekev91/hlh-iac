...
usage() {
	cat <<'EOF'
Usage:
	./deploy-hlh-freetoken.sh

This is the direct Proxmox bootstrap path (no OpenTofu):
	1) Create privileged LXC 140 (hlh-freetoken)
	2) Configure GPU passthrough for 890M iGPU (gfx1150)
	3) Start container
	4) Push/run in-container bootstrap script

NOTE: Uses the FreeToken-ROCm fork (Maxritz/FreeToken-ROCm) for native
AMD ROCm support on Radeon 890M (gfx1150/Strix Halo).
EOF
}

LXC_ID=140
LXC_NAME="hlh-freetoken"
LXC_HOSTNAME="hlh-freetoken"
...
confirm_existing_lxc_delete() {
	...
	printf '%s\n' 'Are you sure?  hlh-freetoken is already running and deployed!'
...
pct create "${LXC_ID}" "${LXC_IMAGE}" \
	...
	--description "FreeToken AI engine with ROCm ${ROCM_VERSION} (FreeToken-ROCm fork), model storage on ${POOL}"

echo "[3/6] Adding GPU/ROCm passthrough devices for 890M iGPU..."
...
echo "Access FreeToken API at http://192.168.1.14:1919"
echo "  OpenAI endpoint:    http://192.168.1.14:1919/v1/chat/completions"
echo "  Anthropic endpoint: http://192.168.1.14:1919/v1/messages"
echo "  Health check:       http://192.168.1.14:1919/health"