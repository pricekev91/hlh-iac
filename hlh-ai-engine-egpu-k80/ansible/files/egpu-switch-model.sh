...
# Version: 1.6.1-egpu
# Description: Interactive model switcher for llama.cpp ai-engine service (generic eGPU)
# Auto-pins --device to the detected eGPU regardless of llama.cpp backend (Vulkan/ROCm/CUDA/etc.)
...
# ─── eGPU Detection ──────────────────────────────────────────────────────────
# Auto-detect ANY Vulkan-capable eGPU (RX480, NVIDIA K80, AMD MI60, etc.)
# via llama-server --list-devices. Since the LXC only sees one GPU (the eGPU
# via native PCI passthrough), we simply use the first Vulkan device found.
# Falls back to Vulkan0 if auto-detection fails.
detect_egpu_device() {
  ...
  local dev
  dev=$(echo "${lines[0]}" | sed -n 's/^  \(Vulkan[0-9]\+\):.*/\1/p')
  [ -z "$dev" ] && dev="Vulkan0"
  echo "$dev"
}
...
echo ""
echo "╔══════════════════════════════════════════════════════════════════╗"
echo "║                egpu-switch-model.sh (eGPU generic)             ║"
echo "╠══════════════════════════════════════════════════════════════════╣"
...
echo "  GPU (eGPU)      : $CUR_GPU_DETECTED (auto-pinned)"
echo ""
...
# ─── Summary & confirm ─────────────────────────────────────────────────────────
echo ""
GPU_FLAG="--device $(detect_egpu_device 2>/dev/null || echo Vulkan0)"
...
echo "  GPU         : $GPU_FLAG (eGPU auto-pinned)"
echo ""
